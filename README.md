...
- **[Система импульсно-фазового управления тиристорого выпрямителя] (ссылка не прилагается)**
    - Реализовано на базе STM32
    - Применил АЦП, таймеры, прерывания
    - Реализовал алгоритм детекции 0, скользящего среднего значения
    
- **[Сервисное ПО для настройки ИБП] (ссылка не прилагается)**
    - Реализовано на Qt Creator (C++17/C++23)
    - Чтение/Запись данных на EEPROM микроконтроллера STM32
 
- **[Универсальная библиотека протокола передачи данных по UART для высокого и низкого уровня ПО] (ссылка не прилагается)**
    - Реализовано на C99
    - Разработал легкую универсальную библиотеку для использования верхнего и низкого ПО
      
- **[Логгирование системы и параллельный доступ к ПК по USB] (ссылка не прилагается)**
